...
$loadPackage = mysqli_query($con, "SELECT * FROM packages WHERE id = $id LIMIT 1");

if (mysqli_num_rows($loadPackage) == 0) {
    header("location:packages-services.php");
}

$package = mysqli_fetch_object($loadPackage);

$package_services_id = explode(",", $package->services_id);
                            ...
                            <div class="form-group">
                                ...
                                >
                                    <?php while ($data = mysqli_fetch_assoc($loadServices)) { ?>
                                        <?php if (in_array($data["id"], $package_services_id)) { ?>
                                            <option
